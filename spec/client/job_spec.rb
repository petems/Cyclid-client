...
describe Cyclid::Client::Job do
  context 'when no organization configured' do
    it 'returns helper message instead of raw error' do
      ...
      error_message = "no config found at /tmp/foo/.cyclid/config - Have you selected an organisation with `cyclid organization use <name>`?"
      expect{ @client = Cyclid::Client::Tilapia.new(path: non_existant_path) }.to raise_error(error_message)
    end
  end
